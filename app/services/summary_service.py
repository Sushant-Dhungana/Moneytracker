...
def fetch_business_summary(
    ...
    *,
    ...
    expense = 0.0
    sales = 0.0
    if _business_daily_summary_exists(conn):
        ...
        for postings_relation, entries_relation in relation_pairs:
            ...
            with conn.cursor() as cur:
                cur.execute(
                    f"""
                    select
                      coalesce(sum(case when lp.leg_type in ('sales_revenue', 'income_bucket') and lp.direction = 'credit' then lp.amount else 0 end), 0) as income_total,
                      coalesce(
                        sum(
                          case
                            ...
                            when lp.leg_type = 'inventory_asset'
                              ...
                              and le.txn_type = 'inventory_in' then lp.amount
                            else 0
                          end
                        ),
                        0
